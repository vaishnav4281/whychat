...
export function PersistentChat({ peer, onBack }: Props) {
  ...
  return (
    <div className="w-full h-full flex flex-col glass-card rounded-3xl overflow-hidden">
      {/* Header */}
      <div className="px-4 md:px-5 py-3.5 md:py-4 flex items-center gap-3 border-b border-white/[0.08]">
        <button onClick={onBack} className="w-9 h-9 rounded-full glass flex items-center justify-center hover:bg-white/[0.12] transition shrink-0">
          <ArrowLeft className="w-4 h-4" />
        </button>
        <div className="gradient-border rounded-full shrink-0">
          <img src={peer.avatar} alt="" className="w-10 h-10 md:w-11 md:h-11 rounded-full" />
        </div>
      ...
      {/* Feed */}
      <div ref={feedRef} className="flex-1 overflow-y-auto px-4 md:px-5 py-5 md:py-6 space-y-3">
        {messages.length === 0 && (
      ...
      <div className="px-3 py-3 border-t border-white/[0.08]">
        <div className="glass rounded-full px-3 py-2 flex items-center gap-2">
          ...
          <button onClick={() => fileRef.current?.click()}
            className="w-9 h-9 rounded-full bg-white/[0.08] text-foreground flex items-center justify-center hover:bg-white/[0.14] transition shrink-0">
            <ImagePlus className="w-4 h-4" />
          </button>
          <button onClick={recording ? stopRec : startRec}
            className={`w-9 h-9 rounded-full flex items-center justify-center transition shrink-0 ${
              recording ? "bg-red-500 text-white animate-pulse-dot" : "bg-white/[0.08] text-foreground hover:bg-white/[0.14]"
            }`}>
          ...
          <input
            ...
            className="flex-1 bg-transparent text-sm outline-none px-1 md:px-2 placeholder:text-muted-foreground/50"
          />
          <button onClick={send}
            className="w-10 h-10 rounded-full gradient-premium text-white flex items-center justify-center hover:scale-105 transition shrink-0">
...
function Bubble({ m }: { m: ChatMessage }) {
  ...
  const base = "max-w-[75%] rounded-3xl px-4 py-2.5 text-sm animate-in shadow-sm";
  const skin = mine
