...
import {
  COUNTRIES, LANGUAGES, flagFor, generateMockPeers, generateMockRequests,
  loadFriends, addFriend, wsStub,
  type UserProfile, type PeerUser, type FriendRequest,
} from "@/lib/peerStore";
...
export function ExploreDashboard({ profile, session, onSessionChange, onLogout, onOpenChat }: Props) {
  const allPeers = useMemo(() => generateMockPeers(20), []);
  const [requests, setRequests] = useState<FriendRequest[]>(() => generateMockRequests(3));
  const [friends, setFriends] = useState<PeerUser[]>(() => loadFriends());
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setOnline((n) => n + Math.floor(Math.random() * 7 - 3)), 2000);
    return () => clearInterval(t);
  }, [profile]);

  const filtered = useMemo(() => {
    wsStub.fetch_explore({ genderFilter, countryFilter, langFilter });
  ...
  const acceptRequest = (r: FriendRequest) => {
    wsStub.accept_request(r.id);
    const next = addFriend(r.from);
    setFriends(next);
  ...
  const sendReq = (p: PeerUser) => {
    wsStub.send_request(p.id);
    setSentTo((prev) => new Set(prev).add(p.id));
  };
              ...
              <div className="space-y-2">
                ...
                {requests.map((r) => (
                  <div key={r.id} className="glass-strong rounded-2xl p-3 flex items-center gap-3 animate-fade-up">
                    <img src={r.from.avatar} alt="" className="w-11 h-11 rounded-full bg-white/60" />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold truncate">{r.from.nickname}</div>
                      <div className="meta-label !text-[10px]">{flagFor(r.from.country)} {r.from.country}</div>
                    </div>
              ...
              <div className="space-y-2">
                ...
                {friends.map((f) => (
                  <button key={f.id} onClick={() => onOpenChat(f)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold truncate">{f.nickname}</div>
