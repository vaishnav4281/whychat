...
const BOT_REPLIES = {
  'bot-sophia': [
    "Hey! Thanks for reaching out. I'm Sophia. What brings you to MeetWithStrangers?",
...
export class SwitchboardDO {
  ...
  handleBotMessage(bot, raw) {
    ...
    if (type === 'FRIEND_REQ') {
      ...
      this.send(sender, 'FRIEND_ACCEPT', {
  ...
  queueBroadcast() {
    ...
    queueMicrotask(() => {
      ...
      const payload = { online, totalVisits: this.totalVisits };
      for (const c of this.clientsBySocket.values()) {
  ...
  onMessage(client, raw) {
    ...
    switch (msg.type) {
      ...
      case 'join_pool': {
        ...
        if (isNew) {
          this.totalVisits++;
          console.log('[MeetWithStrangers] Total account creations:', this.totalVisits);
        }
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/health') {
      return Response.json(
        { ok: true, online: this.clientsBySocket.size, queued: this.videoQueue.size, totalVisits: this.totalVisits },
        { headers: CORS }
      );
    }
    ...
    return new Response('MeetWithStrangers switchboard DO is running.', {
